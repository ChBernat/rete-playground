...
import ConnectionPathPlugin, {getTransformAlong } from 'rete-connection-path-plugin';
...
console.log(ConnectionPathPlugin)
    ...
    editor.on('renderconnection', ({ el }) => {
        ...
        setInterval(() => {
            position=(position + .01)%1;
            marker.setAttribute('transform', getTransformAlong(path, 0, position));
        }, 1000/60)
      });
